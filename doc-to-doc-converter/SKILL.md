...
2. **调用工具**：对每个目标文件执行：
   ```bash
   bash $CURSOR_SKILLS/doc-to-doc-converter/scripts/md2doc.sh "path/to/file.md"
   ```
...
- 若 `$CURSOR_SKILLS/doc-to-doc-converter/scripts/md2doc.sh` 不存在或执行失败，只报错并停止，不尝试替代实现。
